...
// === FUNÇÃO UNIVERSAL PARA CAMPOS JIRA ===
const extrairValor = (campo) => {
  if (!campo) return null;

  if (campo.type === "doc") return extrairTextoRich(campo);

  if (typeof campo === "string") return campo.trim();

  if (typeof campo === "number" || typeof campo === "boolean") return campo;
...
async function buscarIssues(projeto) {
  ...
  try {
    ...
    const issues = await Promise.all(
      (data.issues || []).map(async (issue) => {
        ...
        return {
          ...
          data_inicio: f.customfield_14439 || null,

          data_resolucao: f.customfield_14440 || null,
          ...
          criterioElegibilidade: extrairValor(f.customfield_14444),
    ...
    console.error("Erro ao buscar issues:", err.response?.data || err.message);
